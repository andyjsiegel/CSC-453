...
public class Parse {
    ...
    EXPR factor() {
        ...
        if (lookahead(Token.IDENT)) {
           ...
           if (lookahead(Token.LBRACK)) {
              ...
              EXPR e = expr();
              match(Token.RBRACK);
              expr = new INDEX(ident, e, currentToken.position);
           } else {
              expr = new IDENT(ident, currentToken.position);
           }
   ...
   DECL typeDecl() {
        ...
        EXPR e = expr();
        match(Token.OF);
        String elmtType = currentToken.ident;
        match(Token.IDENT);
        return new ARRAY(ident, elmtType, e, currentToken.position);
   }
   ...
   STAT assignStat() {
        ...
        match(Token.EQUAL);
        EXPR right = expr();
        return new ASSIGN(left, right, currentToken.position);
   }
