/* Copyright 2002, Christian Collberg, collberg@cs.arizona.edu. */
...
public class Lex {
    ...
    Token scanNumber() {
        String s = ""; 
        while ((!done) && (Character.isDigit(ch) || (ch == '.'))) {s+=ch; get();}
        try {
           ...
           try {
              ...
              return new Token(Token.FLTLIT, pos(), fval);
           } catch (NumberFormatException e2) {
             System.err.println("Illegal number format " + s);
           }
        };
        return null;
    }
    ...
    Token scanName()  {
        ...
        else if (ident.equals("VAR") )  return new Token(Token.VAR, pos());
        else if (ident.equals("CONST") ) return new Token(Token.CONST, pos());
        else if (ident.equals("ARRAY") ) return new Token(Token.ARRAY, pos());
        else if (ident.equals("OF") )   return new Token(Token.OF, pos());
        else if (ident.equals("TYPE") )   return new Token(Token.TYPE, pos());
    ...
    public Token nextToken() {
        ...
        switch (ch) {
            ...
            case '*': get(); return new Token(Token.MULT, pos()); 
            case '/': get(); return new Token(Token.DIV, pos()); 
            ...
            case '[': get(); return new Token(Token.LBRACK, pos());
            case ']': get(); return new Token(Token.RBRACK, pos());
