   ...
   void translate() {
      ...
      switch (prog[pc]) {
         case IR.HEADER    : {
          ...
          case IR.BRANCH : {
            int target = prog[pc+1];         // The branch target label
            String cond = pop();             // The condition register
            add("\tbne\t" + cond + ",$zero,L" + target);  // Branch if not equal to zero
            pc+=2; 
            break;
          }
          case IR.LABEL : {
            int label = prog[pc+1];          // The label number
            add("L" + label + ":");          // Declare the label
            pc+=2;
            break;
          }
          case IR.GOTO : {
            int target = prog[pc+1];         // The jump target label
            add("\tj\tL" + target);          // Unconditional jump
            pc+=2;
            break;
          }
          case IR.LOAD   : {
