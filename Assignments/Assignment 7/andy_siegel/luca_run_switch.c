...
/*************************************************************/
/* Sizes of instructions and data.                           */
/*************************************************************/
...
void interpret(char prog[]) {
   ...
   char* globalMem = calloc(globalSize + 16, 1);

   CallFrame callStack[CALL_STACK_SIZE];
   int callDepth = 0;
   char* currentFrame = NULL;
   int   currentFormalSz = 0;

   union stackNode actuals[ACTUALS_MAX];
   int actualsCount = 0;
   ...
   while(1) {
      ...
      switch (op) {
         ...
         case PROC_END: {
            callDepth--;
            free(currentFrame);
            currentFrame    = callStack[callDepth].prevFrame;
            currentFormalSz = callStack[callDepth].prevFormalSz;
            pc              = callStack[callDepth].returnPC;
            break;
         }
         /**********************************************/
         /* Designators.                               */
         ...
         case PUSHADDR_local: {
            ...
            stack[sp].a = currentFrame + currentFormalSz + offset;
            pc += PUSHADDR_SIZE;
            sp++;
            break;
         }
         case PUSHADDR_formal: {
            INT_TP offset = (*(INT_TP*)(&prog[pc+OP_SIZE]));
            stack[sp].a = currentFrame + offset;
            pc += PUSHADDR_SIZE;
            sp++;
            break;
         }
         ...
         case ACTUAL_i:
         case ACTUAL_f: {
            actuals[actualsCount++] = stack[--sp];
            pc += ACTUAL_SIZE;
            break;
         }
         case CALL: {
            INT_TP relAddr     = *(INT_TP*)(&prog[pc+OP_SIZE]);
            INT_TP callFormalSz = *(INT_TP*)(&prog[pc+OP_SIZE+ARG_SIZE]);
            int procAddr = pc + (int)relAddr;

            INT_TP localSz = *(INT_TP*)(&prog[procAddr+OP_SIZE+ARG_SIZE]);

            char* newFrame = calloc((int)(callFormalSz + localSz) + 16, 1);

            for (int k = 0; k < actualsCount; k++) {
               memcpy(newFrame + k * ARG_SIZE, &actuals[k], ARG_SIZE);
            }
            actualsCount = 0;

            callStack[callDepth].returnPC     = pc + CALL_SIZE;
            callStack[callDepth].prevFrame    = currentFrame;
            callStack[callDepth].prevFormalSz = currentFormalSz;
            callDepth++;

            currentFrame    = newFrame;
            currentFormalSz = (int)callFormalSz;
            pc = procAddr;
            break;
         }
         /**********************************************/
